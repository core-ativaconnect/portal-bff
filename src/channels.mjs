...
export async function channelOperation(store,operation,body,params,actor,contract) {
  ...
  if(operation==='create') {
    ...
    const item=channelItem({id:randomUUID(),contract_id:contract.id,name,slug,type,status:type==='WEBCHAT'?'CONNECTED':'UNASSIGNED',webchat_agent_name:agent,created_at:now()});
  ...
  const updated=channelItem({...channel,name:required(body.name,'Nome',180)});
