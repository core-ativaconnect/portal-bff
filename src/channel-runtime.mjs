...
export async function customerHandoff(store,contract,channel,contact,input,ticket,operationId){
  ...
  await store.put('helpdesk_tickets',{...ticket,status:'CLOSED',close_reason:'CUSTOMER',closed_at:now(),updated_at:now()},{previous:ticket,contractId:contract.id});
...
export async function runContactFlow(store, contract, channel, contact, input, operationId) {
  ...
  const tickets = await store.list('helpdesk_tickets', t => t.channel_id === channel.id && t.contact_id === contact.contact_id && t.status !== 'CLOSED');
  ...
  const session = await store.get('engine_sessions', {session_key: `${contact.contact_id}#${target.flow.id}#${target.versionId}`})
    ?? (await store.list('engine_sessions',s=>s.simulator_user_id===contact.contact_id&&s.flow_id===target.flow.id&&s.version_id===target.versionId&&s.contract_id===contract.id))[0];
  ...
  if (response.waitingState === 'HUMAN_HANDOFF') {
    ...
    const queues = await store.list('contract_help_desk_queues', q => q.contract_id === contract.id && q.enabled);
    ...
    await store.put('helpdesk_tickets', {id, contract_id: contract.id, contract_slug: contract.slug,
