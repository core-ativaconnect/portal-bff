...
export const commandCatalog=[...routes,...extraRoutes].map(route=>{
  ...
  if(definition.query.length)samplePath+='?'+new URLSearchParams(definition.query.filter(q=>!['cursor','contactId'].includes(q.name)).map(q=>[q.name,q.name==='month'?'2026-09':q.name==='paged'?'true':q.name==='limit'?'50':q.name==='path'?'/pagina-exemplo':'empresa-exemplo']));
