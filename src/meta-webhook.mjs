...
export async function deliveries(store, payload, raw, signature, secrets) {
  ...
  for(const entry of payload.entry){
    const waba=must((await store.list('whatsapp_wabas',w=>w.waba_id===entry.id))[0]);
    ...
    for(const change of entry.changes??[]){
      ...
      const phone=must((await store.list('whatsapp_phone_numbers',p=>p.meta_phone_number_id===phoneId&&p.waba_config_id===waba.id))[0]);
...
export async function handler(event,_context,dependencies={}){
  ...
  try{
    ...
    if(method==='GET'){
      ...
      const matches=await store.list('whatsapp_apps',a=>same(a.verify_token,query['hub.verify_token']));
...
export async function processDelivery(delivery,store=new Store(),dependencies={}){
  ...
  const channel=(await store.list('contract_channels',c=>c.type==='WHATSAPP'&&c.whatsapp_phone_number_id===phone.id))[0];
  if(!channel)return;
  const contract=must(await store.get('contracts',{id:channel.contract_id}));
  if(delivery.status){
    const status=delivery.status;
    const rows=await store.list('engine_messages',m=>m.channel_id===channel.id&&m.message_id===status.id&&m.direction==='OUTBOUND'&&m.contact_wa_id===status.recipient_id);
    const ranks={SENT:1,DELIVERED:2,READ:3,FAILED:4},next=String(status.status).toUpperCase();
    for(const row of rows){
      if(!(next in ranks)||ranks[next]<(ranks[row.status]??0))continue;
      await store.transaction([store.guard(contract.id),store.putOperation('engine_messages',{...row,status:next,status_payload_json:JSON.stringify(status),updated_at:now()})]);
    }
    return;
  }
  ...
  try{
    ...
    if(!existing){
      await store.transaction([store.guard(contract.id),store.putOperation('engine_messages',{
        ...
        occurred_at:message.timestamp?new Date(Number(message.timestamp)*1000).toISOString():now(),updated_at:now()},'attribute_not_exists(message_id)')]);
    }
    if(!job.processed){
    ...
    for(let index=job.sent_count;index<job.messages.length;index++){
      ...
      await (dependencies.send??sendText)(store,contract,channel,contact,outgoing.text||'Continuando atendimento.','BUSINESS',outgoing);
